...
function printTable(){
  ...
  data["results"][0].members.forEach(element => {
    ...
    let infoTabla = [fullName, element.party, element.state, element.seniority, element.votes_with_party_pct + "%"];
    //aplico los filtros
    if(filtra(element.party) && (state==element.state || state=="All")){
      var datos = createTrBody(infoTabla);
      table.appendChild(datos);
    }
  });
...
function filtra(string){
  let insert = false;
  if((filters["republican"] && string =="R") || (filters["democrat"] && string =="D") || (filters["independent"] && string == "I")){
    insert = true;
  }
  return insert;
}

function creaDrowpdown(){
data["results"][0].members.forEach(element => {
  if(!superarray.includes(element.state)){
    superarray.push(element.state);
  }
});
  superarray.sort();
