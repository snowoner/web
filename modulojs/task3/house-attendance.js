...
function fillLeastEngaged() {
  let table = document.getElementById("houseLeastEngaged");
  let template = "";
  let values = theNLowest(stadistics.republicans.concat(stadistics.democrats, stadistics.independents), 10);
  for (let i = 0; i < values.length; i++) {
    ...
    <tr>
      ...
      <td>${values[i].missed_votes}</td>
      <td>${values[i].missed_votes_pct}</td>
    </tr>
    `;
    }
  }
...
function fillMostEngaged() {
  let table = document.getElementById("houseMostEngaged");
  let template = "";
  let values = theNGreatest(stadistics.republicans.concat(stadistics.democrats, stadistics.independents), 10);
  for (let i = 0; i < values.length; i++) {
    if(values[i].total_votes!=0){
    template += `
    <tr>
